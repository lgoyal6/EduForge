...
describe("event bridge", () => {
  ...
  it("falls back to the local collector when no bus is registered", () => {
    expect(resolveEventSink()).toBeNull();
    const { state } = createRun();
    expect(localEvents(state.run_id)).toHaveLength(6);
  });

  it("forwards to a bus registered by Person D", () => {
  ...
  it("picks up a bus published on the documented global slot", () => {
    const received: AgentEvent[] = [];
  ...
  it("keeps the run alive when the bus throws", () => {
    registerEventBus({
      publish: () => {
