...
describe("core loop", () => {
  ...
  it("runs upload to variants and reaches variants_ready", () => {
  ...
  it("emits the six core loop events in order", () => {
    const { state } = createRun();
  ...
  it("mirrors every event into the local fallback collector", () => {
    const { state } = createRun();
  ...
  it("returns AgentResult envelopes that pass Zod validation", () => {
    const { results } = createRun();
  ...
  it("is deterministic: same input against a fresh store yields identical output", () => {
  ...
  it("builds the four named rooms from the seed cohort", () => {
    const { state } = createRun();
  ...
  it("keeps a clean review queue when every check passes", () => {
    const { state } = createRun();
    expect(state.review_queue).toEqual([]);
  });

  it("rejects an unknown assignment id", () => {
    expect(() => createRun({ assignment_id: "asg_does_not_exist" })).toThrow(
      AssignmentNotFoundError,
    );
  });

  it("accepts a teaching intent override", () => {
    const { state } = createRun({ teaching_intent: "Focus on sign accuracy." });
