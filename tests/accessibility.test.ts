...
describe("accessibility agent", () => {
  ...
  it("creates one delivery layer per student", () => {
    const { state } = createRun();
    expect(state.accessibility?.layers).toHaveLength(seedStudents.length);
  });

  it("asserts it is a delivery layer only", () => {
    const { state } = createRun();
  ...
  it("derives every directive from a documented support", () => {
    const { state } = createRun();
  ...
  it("routes every support to a presentation, pacing, visibility, or sequencing channel", () => {
  ...
  it("leaves students without documented supports on standard delivery", () => {
    const { state } = createRun();
  ...
  it("does not keep a student with documented supports out of Summit", () => {
    const { state } = createRun();
