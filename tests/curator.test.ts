...
describe("assignment curator", () => {
  ...
  it("produces one variant per room", () => {
  ...
  it("preserves every objective in every variant", () => {
    const { state } = createRun();
  ...
  it("keeps the same item count as the original assignment", () => {
    const { state } = createRun();
    for (const variant of state.variants) {
  ...
  it("never lowers rigour and stays inside tolerance", () => {
    const { state } = createRun();
    for (const variant of state.variants) {
  ...
  it("changes the pathway, not the prompt", () => {
    const { state } = createRun();
  ...
  it("raises rigour for Summit and holds it steady elsewhere", () => {
    const { state } = createRun();
    for (const variant of state.variants) {
  ...
  it("attaches a presentation-only overlay for every room member", () => {
    const { state } = createRun();
    for (const variant of state.variants) {
  ...
  it("fails the objective check when an objective is dropped", () => {
    const items = [
      {
        item_id: "x1",
  ...
  it("fails the rigour check when the bar is lowered", () => {
  ...
  it("emits assignment.variants.ready with both checks passing", () => {
    const { state } = createRun();
