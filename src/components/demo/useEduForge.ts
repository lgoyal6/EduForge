...
export type EduForgeController = ReturnType<typeof useEduForge>;

export function useEduForge() {
  const engineRef = useRef<WorldEngine | null>(null);
  if (engineRef.current === null) {
    engineRef.current = new WorldEngine({ minEventSpacing: 0.45 });
  }
  ...
  const [assignmentText, setAssignmentText] = useState(mockAssignment.source_text);
  const [teachingIntent, setTeachingIntent] = useState(mockAssignment.teaching_intent);
