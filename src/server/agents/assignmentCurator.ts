...
export function runAssignmentCurator(
  ...
  const confidence = round4(checksPassed ? 0.94 : 0.45);

  const result = buildAgentResult(variantBundleSchema, {
    run_id: ctx.run_id,
    agent: AGENT,
    confidence,
    evidence_refs: bundle.variants.flatMap((v) => v.evidence_refs),
    result: bundle,
    human_review_required: !checksPassed,
  });
  ...
  ctx.emit("assignment.variants.ready", AGENT, {
    ...
    variants: bundle.variants.map((v) => ({
      ...
      focus_concepts: v.focus_concepts,
