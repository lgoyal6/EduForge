...
export function simulateSubmissions(runId: string): RunState {
  let run = getOrCreateRun(runId);
  if (run.status === "assessed" || run.status === "planned") {
    // Re-running the demo: rebuild from seed so the outcome is identical.
    resetRun(runId);
    run = getOrCreateRun(runId);
  }
  ...
  // 3. Lesson Planner drafts tomorrow's timeline.
