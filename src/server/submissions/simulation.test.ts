...
describe("simulateSubmissions", () => {
  ...
  it("emits every contract event in CONTRACTS.md order", () => {
    simulateSubmissions(RUN_ID);
    const sequence = getRunEvents(RUN_ID).map((event) => event.event_type);
    expect(sequence).toEqual([...eventTypes]);
  });

  it("emits assessment-phase events from the owning agents", () => {
    simulateSubmissions(RUN_ID);
  ...
  it("grades all 15 students deterministically and reaches planned status", () => {
  ...
  it("leaves Forge as the strictly largest room and largest remaining gap", () => {
    const run = simulateSubmissions(RUN_ID);
  ...
  it("keeps every room populated and Ember shrunk after the intervention", () => {
    const run = simulateSubmissions(RUN_ID);
  ...
  it("is deterministic across a reset and across re-simulation", () => {
    const first = domainSnapshot(simulateSubmissions(RUN_ID));

    // Re-simulate on a planned run: orchestrator resets to seed internally.
    const second = domainSnapshot(simulateSubmissions(RUN_ID));
    expect(second).toBe(first);

    resetRun(RUN_ID);
    const third = domainSnapshot(simulateSubmissions(RUN_ID));
    expect(third).toBe(first);
  });

  it("records review gates for the held grade and the final plan", () => {
    simulateSubmissions(RUN_ID);
...
describe("approvePlan", () => {
  ...
  it("returns run_not_found for unknown runs", () => {
    const result = approvePlan("missing-run");
    expect(result).toEqual({ ok: false, error: "run_not_found" });
  });

  it("approves the plan, resolves the plan gate, and audits the decision", () => {
    simulateSubmissions(RUN_ID);
