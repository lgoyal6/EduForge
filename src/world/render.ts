...
import { WORLD_LAYOUT, centerOf, groundBounds } from "./layout";
import type {
  ActorState,
  BuildingSpec,
  BuildingState,
...
function drawGround(ctx: CanvasRenderingContext2D, state: WorldState): void {
  ...
  for (let x = bounds.minX; x <= bounds.maxX; x += 1) {
    for (let y = bounds.minY; y <= bounds.maxY; y += 1) {
      ...
      const base = checker ? "#2c3a5c" : "#27334f";
...
/** Windows and banners on the two visible vertical faces. */
function drawFaceDetail(
  ...
  for (let row = 0; row < rows; row += 1) {
    for (let column = 0; column < columns; column += 1) {
      ...
      ctx.fillRect(px(leftX - 1), px(leftY + yOffset), 3, 4);
      ...
      ctx.fillRect(px(rightX - 1), px(rightY + yOffset), 3, 4);
    }
  }
